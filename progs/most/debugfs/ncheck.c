		       ...
		       void	*private)
{
	...
	for (i=0; i < iw->num_inodes; i++) {
		if (iw->iarray[i] == dirent->inode) {
			...
			sprintf(data_name, "D\t%s/%.*s", iw->parent,(dirent->name_len & 0xFF), dirent->name);
			#else
...
void mod_ncheck(__u32 num_inode)
{
...
void do_ncheck(int argc, char **argv)
{
	...
	while (ino) {
		...
		printf("Inode\tPathname1 \n");
		retval = ext2fs_dir_iterate(current_fs, ino, 0, 0,
					    ncheck_proc, &iw);

		
		printf("Inode\tPathname2 \n");
