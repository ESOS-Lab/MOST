...
#define BLKTRACE_DEBUGFS


#ifdef BLKTRACE_DEBUGFS
#define BLKTRACE_DEBUGFS_SDCARD

//#define BLKTRACE_DEBUGFS_PART1 //PARTITION 1
//#define BLKTRACE_DEBUGFS_PART2 //PARTITION 2

#define JOURNAL_INODE 8

#if defined BLKTRACE_DEBUGFS_SDCARD
#define PARTITION_BLOCK_START 256
#elif defined BLKTRACE_DEBUGFS_PART1

#define PARTITION_BLOCK_START 256
#define PARTITION_BLOCK_SIZE 131072
#define PARTITION_BLOCK_END (PARTITION_BLOCK_START+PARTITION_BLOCK_SIZE-1)

#define GROUP_NUM 4

static int group_data_range[GROUP_NUM][2]=
{
	{547, 32768},
	{33315, 65503},
	{66050, 98304},
...
#elif defined BLKTRACE_DEBUGFS_PART2
#define PARTITION_BLOCK_START 131328
#define PARTITION_BLOCK_SIZE 262144
#define PARTITION_BLOCK_END (PARTITION_BLOCK_START+PARTITION_BLOCK_SIZE-1)
#define GROUP_NUM 8
...
int main_orig(int argc, char **argv)
{
	...
	if(argv[2]!=NULL)
		block = strtoull(argv[2], &tmp, 0);

	if(block!=0)
		inode = mod_icheck(block);
...
int debugfs_open_filesystem()
{
	...
	printf("debugfs_open_filesystem\n");

	#ifdef BLKTRACE_DEBUGFS_SDCARD
	open_filesystem("/dev/block/mmcblk1p1", open_flags,
			superblock, blocksize, catastrophic,
			data_filename);

	#else
 
	#ifdef BLKTRACE_DEBUGFS_PART1
	open_filesystem("/dev/block/mmcblk0p1", open_flags,
			superblock, blocksize, catastrophic,
			data_filename);
	#else
	open_filesystem("/dev/block/mmcblk0p2", open_flags,
			superblock, blocksize, catastrophic,
			data_filename);
	#endif
	#endif
...
int debugfs_close_filesystem()
{
	printf("debugfs_close_filesystem\n");
...
#ifdef BLKTRACE_DEBUGFS
#ifndef BLKTRACE_DEBUGFS_SDCARD
static int is_data_block(block)
{
	int i =0;
	int result = 0;
...
int debugfs(__u64 block)
{
	...
	if(inode == 0)
	{
		//일단 Metadata로 설정함. Temp 파일인 경우 따로 처리함.
		sprintf(data_name,"%s", "M");
		//sprintf(data_name,"%s\tTempfiles", "D");
	}
	else if(inode == JOURNAL_INODE)
	{
		//fprintf(ofp, "J");
		sprintf(data_name,"%s", "J");
	}
	else
		mod_ncheck(inode);
